...
## Team completion command

Run the final clean pipeline, extended retrieval eval, and official grading:
...
## 4-Sprint completion evidence

| Sprint | Status | Evidence |
|---|---|---|
| Sprint 1 - Analyze & ingest | Complete | `artifacts/logs/run_optimized-final.log`, `docs/data_contract.md` |
| Sprint 2 - Clean, validate, embed | Complete | `artifacts/manifests/manifest_optimized-final.json`, `artifacts/eval/grading_run.jsonl` |
| Sprint 3 - Inject & before/after | Complete | `artifacts/eval/after_inject_bad.csv`, `artifacts/eval/eval_optimized_final.csv`, `docs/quality_report.md` |
| Sprint 4 - Monitor, docs, report | Complete | `docs/pipeline_architecture.md`, `docs/runbook.md`, `reports/group_report.md` |

## Multi-Agent RAG hỏi đáp

Hỏi trực tiếp collection sạch `day10_kb` qua Supervisor, Retrieval Agent,
Quality Guard Agent và Synthesis Agent:

```powershell
python multi_agent_rag.py "Level 4 Admin Access yêu cầu ai phê duyệt?" --trace
...
Chi tiết kiến trúc và guardrail: [`docs/multi_agent_rag.md`](docs/multi_agent_rag.md).

Báo cáo HTML tổng hợp: [`project_summary.html`](project_summary.html), hoặc mở
`http://localhost:8000/report` khi web server đang chạy.
